...
!private :: readrecI, readrecR
!private :: readlineI, readlineR

INTERFACE OPERATOR (+)
 MODULE PROCEDURE concat
END INTERFACE

interface Numvalue  ! Generic operator for converting a number string to a 
...
interface readline
  module procedure readlineR, readlineI
end interface

interface readrec
  module procedure readrecI, readrecR
end interface
...
!**********************************************************************
 ...
 SUBROUTINE readlineI(unt,n,XC,XI,dlm,delc)
   INTEGER:: unt,n
   integer:: XI(:)
   real:: XX(size(XI))
   CHARACTER(LEN=*):: XC(:)
   CHARACTER(LEN=1),optional:: dlm
   logical,optional:: delc
   CHARACTER(LEN=1):: delim=achar(32)
   logical:: delcc=.false.
   integer:: ii

   if(present(dlm)) delim=dlm
   if(present(delc)) delcc=delc
   call readlineR(unt,n,XC,XX, dlm=delim, delc=delcc)
   do ii=1,n
        XI(ii)=int(XX(ii))
   enddo
 END SUBROUTINE

 SUBROUTINE readlineR(unt,n,XC,XX,dlm,delc)
   INTEGER:: unt, n
   CHARACTER(LEN=*):: XC(:)
   character(len=MAX_RECL):: RECD
   real:: XX(:)
   character(len=1),optional:: dlm
   logical,optional:: delc
   character(len=1):: delim=achar(32)
   ...
   if(present(dlm)) delim=dlm
   if(present(delc)) delc=delcc
   XC=missC
   XX=missR
   n=missI
   DO WHILE(n == 0)
     READ(unt,'(a)',IOSTAT=io) RECD
     IF (io /= 0) THEN
         n=-1
         RETURN
     ENDIF
     RECD = trim(ADJUSTL(RECD))
     n = len_trim(RECD)
     step = SCAN(RECD,'#')
     IF (step .eq. 1) n=0
   ENDDO
   IF (step > 1) RECD = RECD(1:step-1)
   if(delcc) call delchar(RECD,delim)
   IF (n.GT.0) CALL VAL_num(RECD,n,XC,XX,delim)
 END SUBROUTINE

 subroutine delchar(a,dlm)
  ...
  do i=1,N
     if(a(i:i) == dlm) cycle
     k=iachar(a(i:i))
     if((k<=44).or.(k==47)) then
         a(i:i)=missC
     elseif((k>=58).and.(k<=64)) then
         a(i:i)=missC
     elseif((k>=91).and.(k<=96)) then
         a(i:i)=missC
     elseif(k>=123) then
         a(i:i)=missC
     endif
 ...
 SUBROUTINE VAL_num(a,n,XC,X,dlm) 	! separates array a into items delimited ichar(" ")
  CHARACTER(LEN=*), INTENT(IN):: a
  INTEGER::n
  CHARACTER(LEN=*):: XC(:)
  real::X(:)
  character(len=1):: dlm
  integer:: SEP(0:MAX_RECL)
  INTEGER:: i, cnt, len_a

  SEP=0
  len_a=len(a)
  if(ichar(a(len_a:len_a)).eq.13) len_a=len_a-1  ! ichar(CR)=13
  ...
  n=0
  if(dlm==achar(32)) then ! seperated by space__ ignore trail space
      do i=0, cnt-1
           if((SEP(i+1)-SEP(i))>1) then
              n=n+1
              XC(n)=a(SEP(i)+1:SEP(i+1)-1)
           endif
      enddo
  else                                         ! seperated by else other__ consider consecutive delimiter
      do i=0, cnt-1
           n=n+1
           if((SEP(i+1)-SEP(i))>1) then
              XC(n)=a(SEP(i)+1:SEP(i+1)-1)
  ...
  if(n==missI) return
  do i=1,n
          XC(i)=trim(adjustl(XC(i)))
          if(UPPER(XC(i)).eq."ANIMAL") XC(i)="ARN"
          if(UPPER(XC(i)).eq."SIRE") XC(i)="SRN"
          if(UPPER(XC(i)).eq."DAM") XC(i)="DRN"
          X(i) = STR_R(XC(i))
  enddo
 END SUBROUTINE

 elemental function upper(strIn) result(strOut)
 ...
 elemental character(len=1) function chstr(nu) result(ch)
 integer(kind=ki1),intent(in):: nu
 ch=char(nu+48)
 endfunction
...
elemental character(len=LEN_STR) function I_STR(a)
 integer,intent(in):: a
 integer:: error
 write(I_STR,fmt="(i15)",iostat=error) a
 if(error/=0) I_STR=" "
 I_STR=adjustl(I_STR)
 I_STR=trim(I_STR)
 END function

elemental real FUNCTION STR_R(str)
 CHARACTER(LEN=*),intent(in):: str
 character(LEN=len(str)):: st
 integer:: error
 st=adjustl(str)
 read(st,*,iostat=error) STR_R
 if(error.ne.0) STR_R=0.d0
END FUNCTION


!===========================================
! read record for opened file
!===========================================
 subroutine readrecI(unt,MAX_REC,nargs,XC,XI,dlm)
 integer,intent(in):: unt, MAX_REC
 integer, intent(out):: nargs
 character(len=*),intent(inout):: XC(:)
 integer,intent(inout):: XI(:)
 character(len=1),optional, intent(in):: dlm
 character(len=MAX_REC):: RECD
 character(len=1):: dlm0=achar(32)
 integer:: ios, istep
 if(present(dlm)) dlm0=dlm
 XC=' '
 XI=0
 nargs=0
 call readlineS(unt,RECD,ios)
 if(ios/=0)then
      nargs=0
     return
 endif
 call parse(RECD,dlm0,XC,nargs)
 if(nargs<1) return
 do istep=1,nargs
       call Numvalue(XC(istep),XI(istep),ios)
 enddo
 end subroutine

 subroutine readrecR(unt,MAX_REC,nargs,XC,XR,dlm)
 integer,intent(in):: unt, MAX_REC
 integer, intent(out):: nargs
 character(len=*),intent(inout):: XC(:)
 ...
 if(nargs<1) return
 do istep=1,nargs
       call Numvalue(XC(istep),XR(istep),ios)
 enddo
 end subroutine


!===========================================
! get unit number, record length, file open, N- record on file
!===========================================
   FUNCTION Get_unit() RESULT( Lunit )
   INTEGER :: Lunit
   LOGICAL :: Is_Open
   Lunit=10
   Is_Open = .TRUE.
   DO WHILE(Is_open)
      Lunit = Lunit + 1
      INQUIRE(UNIT = Lunit, OPENED = Is_Open )
   END DO
  END FUNCTION Get_unit

  integer function chkRECL(filename)
  character(len=*):: filename
  character(len=:),allocatable:: recf
  integer :: io,unt
  character(len = 1) :: buffer
  unt=Get_unit()
  open(unt,file=filename,recl=1048576)
  recf=''
  do
     read(unt,fmt='(a)',advance="no", iostat=io) buffer
     if(io/=0) exit
     recf=recf//buffer
  enddo
  chkRECL=len(recf)
 ...
 integer function fopen(filename,rec_size) 
 character(LEN=*),intent(in):: filename
 integer,intent(in), optional:: rec_size
 logical:: F_exists, F_OPENED
 integer:: new_recl

 inquire(file=filename,EXIST=F_exists,OPENED=F_OPENED,number=fopen)
 if(F_exists) then
     if(F_OPENED) close(unit=fopen)
     fopen=Get_unit()
     if(present(rec_size)) then
         open(unit=fopen,file=filename,recl=rec_size,status='unknown')
     else
!         new_recl=chkRECL(filename)
         open(unit=fopen,file=filename,recl=MAX_RECL,status='unknown')
     endif
     return
 else
     fopen=Get_unit()
     if(present(rec_size)) then
         open(unit=fopen,file=filename,recl=rec_size,status='new')
     else
         open(unit=fopen,file=filename,recl=MAX_RECL,status='new')
     endif
     return
 endif
 end function
 ...
 character(LEN=*),intent(in):: filename
 integer,intent(in),optional::  rec_size
 character(LEN=50)::  RECD
 integer:: io, unitF,  NSIZE
 ...
 rec_N=0
 do
   read(unitF,fmt='(a)',iostat=io) RECD
   if(io/=0) exit
   RECD=adjustl(RECD)
   NSIZE=len_trim(RECD)
   if(RECD(1:1)/='#'.or.NSIZE/=0) rec_N=rec_N+1
 enddo
 close(unit=unitF)
 end function N_recf
...
if(nint(abs(rnum),ki8) > huge(inum)) then
  ios=15
  return
end if
inum=nint(rnum,ki8)
...
subroutine readlineS(nunitr,line,ios)
