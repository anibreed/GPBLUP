module renPED
  ...
  character(len=MAX_STR) :: ref_dup_sex_report
...
call cleanup_renumped_state(order, ref_nid, REFAnim, ref_dup_sex_first, ref_dup_sex_dup, ref_dup_sex_id)
 ...
 if (rel_cal) then
   ...
   if (write_rel_txt) then
     write(*,'(A)') 'write_triplets text layout: row col value'
     if (log_unt > 0) write(log_unt,'(A)') 'write_triplets text layout: row col value'
   end if
   call cpu_time(t_inb_start)
    ...
    if (write_rel_txt) then
      if (write_rel_bin) then
        ...
        call write_triplets_text_mod(nrec, sire_arr, dam_arr, F, trim(rel_txt_file))
      end if
    end if
   call cpu_time(t_inb_end)
 ...
 if (relinv_cal) then
   ...
   if (write_relinv_txt) then
     write(*,'(A)') 'write_ainv_triplets text layout: row col value'
     if (log_unt > 0) write(log_unt,'(A)') 'write_ainv_triplets text layout: row col value'
   end if
   call cpu_time(t_inb_start)
    ...
    if (write_relinv_txt) then
      if (write_relinv_bin) then
        ...
        call write_ainv_triplets_text_mod(nrec, sire_arr, dam_arr, F, trim(relinv_txt_file))
      end if
    end if
   call cpu_time(t_inb_end)
 ...
 call initX(X)
  ...
  X(3)%SEX=Female
    ...
    subroutine write_dual_role_report(pedfile, dual_count, report_file)
    character(len=*), intent(in) :: pedfile
    integer(kind=ki4), intent(out) :: dual_count
    ...
    dual_count = 0
    report_file = trim(pedfile)//'.dual_role_ids.txt'
    ...
    write(unt,'(A)') '# IDs that appear as both sire and dam in pedigree'
    write(unt,'(A)') '# id'
    do i = 0, h%n_buckets-1
      if (.not. h%valid_index(i)) cycle
      if (role_mask(i) == 3) then
        dual_count = dual_count + 1
        write(unt,'(A)') trim(h%vals(i)%ASD(1))
      end if
    end do

    close(unt)
    deallocate(role_mask)
    end subroutine write_dual_role_report
    ...
    report_file = trim(reffile)//'.duplicate_sex_conflicts.txt'
    open(newunit=unt, file=trim(report_file), status='replace', action='write', iostat=ios)
    if (ios /= 0) then
      report_file = 'report_open_failed'
      return
  ...
  do k = 1, nnz
    ...
    if (abs(val) > eps) then
      if (row >= 1 .and. row <= nrec .and. col >= 1 .and. col <= nrec) then
        write(u_txt, '(I0,1X,I0,1X,F12.4,1X,A,1X,A)') row, col, val, &
          trim(orig_id(row)), trim(orig_id(col))
      else
        write(u_txt, '(I0,1X,I0,1X,F12.4,1X,A,1X,A)') row, col, val, 'NA', 'NA'
      end if
    end if
  end do
