module M_IO_Ped
  ...
  use M_Variables
  use M_Hashtable
  use M_ped, only: CPED, init_cped_record, cped_sync_links
  implicit none

  type, public :: PEDHashNode
    character(len=LEN_STR) :: ANIMAL_key
    type(CPED) :: ped_data
...
contains
  ...
  subroutine pht_load_ped_file(pht, n_rec, unit_fd, key_name, quiet)
    ...
    if (do_log) then
      print *, " M_PEDHashTable: PED records loading (", trim(PEDFile%FileName), ")"
    end if
  ...
  subroutine pht_create(pht, table_size, quiet)
    ...
    if (do_log) then
      print '(A,I10,A,I10)', "  PED hash table created: requested=", table_size, &
                             "  actual(prime)=", actual_size
    end if
  end subroutine pht_create

  subroutine pht_insert_with_key(pht, ped, key_type)
    type(PEDHashTable), intent(inout) :: pht
    type(CPED), intent(in) :: ped
    ...
    ped_norm = ped
    call cped_sync_links(ped_norm)

    if (index(trim(key_type), 'ID') > 0 .and. index(trim(key_type), 'ARN') == 0) then
      key_value = trim(ped_norm%ID)
    else
      key_value = trim(ped_norm%ARN)
    end if
    if (len_trim(key_value) == 0 .or. trim(key_value) == '0') key_value = trim(ped_norm%ASD(1))
