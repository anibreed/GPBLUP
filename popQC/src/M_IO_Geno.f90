...
contains
  ...
  subroutine load_geno_file_mod(in_n_animals, in_map_records, in_geno_key_field, out_n_snps, &
    ...
    integer :: scan_pos
    integer, allocatable :: geno_pos(:)
    integer :: geno_len_expected
    integer :: geno_len_line
    ...
    if (out_n_snps <= 0) then
      ...
      if (io_stat == 0) then
        ...
        if (found_field) then
          out_n_snps = count_geno_digits(a, geno_s, geno_e)
        end if
      end if
      close(u_fd)
    ...
    if (out_n_snps <= 0) then
      print *, 'ERROR: Unable to determine GENO length (Array_All missing and GENO field empty).'
      return
    end if

    call build_geno_positions(in_map_records, out_n_snps, geno_pos, geno_len_expected)

    if (geno_len_expected <= 0) then
      print *, 'ERROR: No GENO positions matched the MAP/species criteria.'
      return
    end if

    print *, '  MAP ARRAY_ALL SNPs:        ', out_n_snps
    print *, '  GENO digits expected:      ', geno_len_expected
    print *, '  MAP SNPs kept as missing:  ', out_n_snps - geno_len_expected
    ...
    if (qcgenoio_debug) then
      ...
      print *, '  Expected GENO length (MAP criteria): ', geno_len_expected
    end if
    u_fd = fopen(trim(GENOFile%FileName), recl=80000)
    ...
    do i = 1, in_n_animals
      ...
      geno_len_line = 0
      do scan_pos = geno_s, geno_e
        if (a(scan_pos:scan_pos) >= '0' .and. a(scan_pos:scan_pos) <= '9') then
          geno_len_line = geno_len_line + 1
          if (geno_len_line <= geno_len_expected) then
            cv = ichar(a(scan_pos:scan_pos)) - 48
            if (cv >= 0 .and. cv <= 2) then
              io_geno_matrix(i, geno_pos(geno_len_line)) = int(cv, ki1)
            else
              io_geno_matrix(i, geno_pos(geno_len_line)) = 9_ki1
            end if
          end if
        end if
      end do
      ...
      if (qcgenoio_debug .and. i == 1) then
        print *, '  GENO digits in first row: ', geno_len_line
        if (geno_len_line /= geno_len_expected) then
          print *, '  WARNING: GENO length differs from MAP criteria: ', &
                   geno_len_line, ' vs ', geno_len_expected
        end if
      end if
    end do
    ...
    close(u_fd)
    if (allocated(a)) deallocate(a)
    if (allocated(geno_pos)) deallocate(geno_pos)

    print *, '  Finished loading GENO matrix: ', in_n_animals, ' x ', out_n_snps
  end subroutine load_geno_file_mod

  integer function count_geno_digits(line, s, e) result(n_digits)
    ...
    if (e < s) return
    do p = s, e
      if (line(p:p) >= '0' .and. line(p:p) <= '9') n_digits = n_digits + 1
    end do
  end function count_geno_digits

  subroutine build_geno_positions(in_map_records, in_n_snps, out_pos, out_n)
    type(SNPInfo), intent(in) :: in_map_records(:)
    integer, intent(in) :: in_n_snps
    integer, allocatable, intent(out) :: out_pos(:)
    integer, intent(out) :: out_n

    integer :: i, j
    logical, allocatable :: keep_pos(:)
    integer :: chr

    allocate(keep_pos(in_n_snps))
    keep_pos = .false.

    do i = 1, size(in_map_records)
      j = in_map_records(i)%Array_All
      if (j < 1 .or. j > in_n_snps) cycle
      chr = in_map_records(i)%Chr
      if (chr <= 0) cycle
      if (is_autosome_chr(chr)) then
        keep_pos(j) = .true.
      else if (SPECIES_INCLUDE_SEXCHR .and. is_sex_chr(chr)) then
        keep_pos(j) = .true.
      end if
    end do

    out_n = count(keep_pos)
    if (out_n <= 0) then
      if (allocated(out_pos)) deallocate(out_pos)
      deallocate(keep_pos)
      return
    end if

    allocate(out_pos(out_n))
    out_n = 0
    do i = 1, in_n_snps
      if (keep_pos(i)) then
        out_n = out_n + 1
    ...
    deallocate(keep_pos)
  end subroutine build_geno_positions
  ...
  subroutine write_valid_geno_for_imputation_mod(in_n_animals, in_n_snps, in_geno_matrix, in_geno_animal_ids, &
    ...
    n_auto = in_n_snps
    if (present(in_snp_qc_status)) then
      if (size(in_snp_qc_status) >= in_n_snps) then
        n_auto = count(in_snp_qc_status(1:in_n_snps) == 0)
      end if
    end if
