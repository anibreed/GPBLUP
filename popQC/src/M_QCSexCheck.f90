...
contains
  ...
  subroutine detect_x_chromosome_snps(in_map, in_n_map, out_indices, out_n)
    ! Scan map records for X (or Z for ZW species) and collect Array_All indices.
    ...
    if (.not. SPECIES_INCLUDE_SEXCHR) then
      out_n = 0
      return
    end if

    if (SPECIES_USE_ZW) then
      target_chr = SPECIES_CHR_Z
    else
      target_chr = SPECIES_CHR_X
    end if

    if (target_chr <= 0) then
      out_n = 0
      return
    end if
    ...
    cnt = 0
    do i = 1, in_n_map
      if (in_map(i)%Chr == target_chr) cnt = cnt + 1
    end do
    ...
    cnt = 0
    do i = 1, in_n_map
      if (in_map(i)%Chr == target_chr) then
        cnt = cnt + 1
